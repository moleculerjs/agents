...
function convertParam(
	...
	switch (type) {
		...
		case "email":
			result.type = "string";
			result.description =
				(result.description ? result.description + " " : "") + "(format: email)";
			break;

		case "url":
			result.type = "string";
			result.description =
				(result.description ? result.description + " " : "") + "(format: url)";
			break;

		case "date":
			result.type = "string";
			result.description =
				(result.description ? result.description + " " : "") + "(format: date)";
			break;

		case "uuid":
			result.type = "string";
			result.description =
				(result.description ? result.description + " " : "") + "(format: uuid)";
			break;

		case "object": {
			result.type = "object";
			const props = (paramDef.properties || paramDef.props) as
...
export function moleculerParamsToJsonSchema(
	...
	for (const [name, rawDef] of Object.entries(params)) {
		const paramDef =
			typeof rawDef === "string" ? { type: rawDef } : (rawDef as Record<string, unknown>);
